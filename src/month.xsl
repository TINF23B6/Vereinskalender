	...
	<xsl:template match="/">
		...
		<html lang="de">
			...
			<head>
				...
				<link rel="stylesheet" type="text/css" href="src/content_month.css"></link>
						...
						<div class="date-btn option-bar-2" id="date-btn">
							...
							<div class="dropdown-content" id="dropdown-content">
								<div class="month_btns">
									<xsl:for-each select="document('../xml/calendar.xml')/calendar/month_btn">
										<button>
											<xsl:attribute name="onclick">
												<xsl:text>monthBtnClick('</xsl:text>
												<xsl:value-of select="document('../xml/calendar.xml')/calendar/path_month_btn"></xsl:value-of>
												<xsl:text disable-output-escaping="yes">', '</xsl:text>
												<xsl:value-of select="index"></xsl:value-of>
												<xsl:text>')</xsl:text>
											</xsl:attribute>
											<xsl:value-of select="name_month"></xsl:value-of>
										</button>
									</xsl:for-each>
								</div>
					...
					<div class="content">
						<div id="content-calendar">
							<xsl:attribute name="class">
								<xsl:text>content-calendar </xsl:text>
								<xsl:value-of select="document('../xml/calendar.xml')/calendar/calendar_size"></xsl:value-of>
							</xsl:attribute>
							...
							<xsl:for-each select="document('../xml/calendar.xml')/calendar/day_month">
								<div>
									<xsl:attribute name="class">
										<xsl:text>day-</xsl:text>
										<xsl:value-of select="number"></xsl:value-of>
										<xsl:text> calendar-body </xsl:text>
										<xsl:value-of select="in_shown_month"></xsl:value-of>
									</xsl:attribute>
									<div class="events">
										<xsl:for-each select="document('../xml/events.xml')/events/event[date=current()/date]">
									...
									<div class="number">
										...
										<xsl:variable name="today" select="document('../xml/calendar.xml')/calendar/date_today"></xsl:variable>
										<xsl:if test="$date = $today">
											<xsl:attribute name="id">
												<xsl:text>number_today</xsl:text>
											</xsl:attribute>
										</xsl:if>
										<xsl:value-of select="day_number"></xsl:value-of>
									</div>
								</div>
							</xsl:for-each>
						</div>
